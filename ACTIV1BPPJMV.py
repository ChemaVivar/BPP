...
ahorrosmes = newdf[newdf>0].sum()

ahorrosanual = (newdf[newdf>0].sum()).sum()
print(f'Los ingresos totales anuales son {ahorrosanual}')

gastosmes = newdf[newdf<0].sum()

gastoanual = (newdf[newdf<0].sum()).sum()
print(f'Los gastos totales anuales son de {gastoanual}')

media_gastoanual = (newdf[newdf<0].sum()).mean()
print(f'La media de gasto anual es de {media_gastoanual}')

mesmayoringreso = dftotalesmes['Totales positivos'].max()
nombremesmayor = dftotalesmes['Totales positivos'].idxmax()
print(f'El mes de mayor ahorro es {nombremesmayor}, con la cifra de {mesmayoringreso}')

mes_mayorgasto = dftotalesmes['Totales negativos'].min()
nombremesmenor = dftotalesmes['Totales positivos'].idxmin()
